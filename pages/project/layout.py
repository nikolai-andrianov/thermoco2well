...
buttons = html.Div(
    [
        ...
        dbc.Alert(
            "Simulation results not available. Please change the input parameters!", 
            id='run-error-alert', 
            is_open=False, 
            dismissable=True,
            color="danger", 
...
@callback(
    ...
    Output("run-error-alert", "is_open"),
...
def on_run_button_click(set_progress, n, is_open):
    ...
    if n is not None:   
        ...
        if converged:
            ...
            # Return graphs, show the success alert, don't show the run error alert, and activate the Results tab        
            return graphs, not is_open, False, False     

        else:

            # Return no graphs, don't show the success alert, show the run error alert, and don't activate the Results tab        
